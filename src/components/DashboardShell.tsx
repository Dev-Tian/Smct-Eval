...
import { ChevronLeft, ChevronRight, Bell, X, Trash2, ChevronDown, Clock } from 'lucide-react';
...
export default function DashboardShell(props: DashboardShellProps) {
  ...
  useEffect(() => {
    const setupEcho = async () => {
      echo.private(`App.Models.User.${user?.id}`).notification((notification: any) => {
  ...
  useEffect(() => {
    ...
    if (isHRDashboard) {
      if (
        ['departments', 'branches', 'branch-heads', 'area-managers'].includes(activeItemId) &&
        !isManagementOpen
      ) {
        setIsManagementOpen(true);
      }
    ...
    const analyticsItems =
      isEmployeeDashboard || isEvaluatorDashboard ? ['history'] : ['reviews', 'history'];
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    <Clock
                      className={`h-5 w-5 ${isClockVisible ? 'text-gray-500' : 'text-blue-600'}`}
                    />
                  </Button>
                </TooltipTrigger>
        ...
        {isClockVisible && (
          <div
            className="fixed right-5 mt-20 z-40 bg-blue-700 border border-blue-200 rounded-lg shadow-lg px-4 py-2 cursor-pointer transition-colors hover:bg-blue-400"
                  ...
                  {(() => {
                    ...
                    return sidebarItems.map((item) => {
                      ...
                      const analyticsTriggerId =
                        isEmployeeDashboard || isEvaluatorDashboard ? null : 'reviews';
